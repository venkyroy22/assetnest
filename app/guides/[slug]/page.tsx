        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="mb-10">
            ...
            <div className="flex items-start gap-5 mb-6">
              <div className="shrink-0 w-16 h-16 rounded-xl bg-zinc-900 border border-zinc-800 flex items-center justify-center">
            ...
            <p className="text-zinc-400 text-lg leading-relaxed border-l-2 border-zinc-800 pl-5">
              {post.description}
            </p>
          </div>
          ...
          {post.toolLink && (
            <Link
              ...
              className="group flex items-center justify-between gap-4 bg-zinc-950 border border-zinc-800 hover:border-zinc-600 rounded-2xl px-6 py-4 mb-10 transition-all duration-300 hover:shadow-[0_0_40px_rgba(255,255,255,0.04)]"
            >
              <span className="text-sm font-bold text-white group-hover:text-zinc-100 transition-colors">
                {post.toolLink.label}
              </span>
              <div className="shrink-0 flex items-center gap-1.5 text-xs font-bold text-zinc-500 group-hover:text-white transition-colors">
          ...
          {/* Tags */}
          <div className="mt-12 pt-8 border-t border-zinc-900 flex flex-wrap gap-2">
          ...
          {/* Bottom CTA */}
          {post.toolLink && (
            <div className="mt-10 bg-zinc-950 border border-zinc-900 rounded-[1.5rem] p-8 text-center">
              <p className="text-zinc-400 text-sm mb-4">
                Ready to put this guide into practice?
              </p>
              <Link
                href={post.toolLink.href}
                className="inline-flex items-center gap-2 px-6 py-3 bg-white text-black text-sm font-black rounded-xl hover:bg-zinc-100 active:scale-95 transition-all duration-200"
              >
                {post.toolLink.label}
                <ArrowRight size={14} />
              </Link>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {related.map((r) => (
                  <Link
                    key={r.slug}
                    href={`/guides/${r.slug}`}
                    className="group flex items-start gap-4 bg-zinc-950 border border-zinc-900 hover:border-zinc-700 rounded-2xl p-5 transition-all duration-200"
                  >
                    <div className="shrink-0 w-10 h-10 rounded-xl bg-zinc-900 border border-zinc-800 flex items-center justify-center text-xl">
                      {getIcon(r.icon, "w-5 h-5 text-zinc-400 group-hover:text-white transition-colors")}
                    </div>
                    <div>
                      <p className="text-sm font-bold text-white leading-snug group-hover:text-zinc-100 mb-1 transition-colors line-clamp-2">
                        {r.title}
                      </p>
