        ...
        <div className="min-h-[70vh] py-8 px-4 md:px-8 max-w-5xl mx-auto">
            ...
            <div className="text-center mb-10 relative group">
                ...
                <div className="inline-flex items-center gap-2 px-3 py-1 border border-white/[0.07] bg-[#1e1e1e]/50 mb-6">
                    <Scissors size={11} className="text-[#f0ede8]" />
                ...
                <h1 className="text-3xl md:text-5xl font-black tracking-tight text-[#f0ede8] mb-4">
                    PDF <span className="text-[#f0ede8]">Splitter</span>
                </h1>
            ...
            {!file ? (
                ...
                <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom duration-500">
                    {/* Controls bar */}
                    <div className="bg-[#1c1c1c] border border-white/[0.05] rounded-[2rem] p-5 flex flex-col md:flex-row items-start md:items-center gap-4">
                        <div className="flex items-center gap-3 shrink-0">
                            <div className="w-9 h-9 bg-white/10 border border-white/20 rounded-xl flex items-center justify-center">
                                <Split size={16} className="text-[#f0ede8]" />
                            </div>
                            <div>
                                <p className="text-xs font-black text-[#f0ede8] truncate max-w-[200px]">{file.name}</p>
                                <p className="text-[10px] text-zinc-500">{pageCount} pages total</p>
                            </div>
                        </div>

                        <div className="h-px md:h-8 md:w-px bg-white/[0.06] w-full md:w-auto" />
                        ...
                        <div className="flex items-center gap-2 flex-grow">
                            <input
                                ...
                                className="bg-[#1c1c1c] border border-white/[0.07] rounded-xl text-xs font-medium text-zinc-200 px-4 py-2.5 placeholder:text-zinc-600 focus:outline-none focus:border-white/50 flex-grow"
                            />
                            <button onClick={applyRange} disabled={!rangeInput.trim()} className="h-10 px-4 bg-white/10 border border-white/20 text-[#f0ede8] text-xs font-semibold tracking-wide rounded-full hover:bg-white/20 transition-colors disabled:opacity-40 disabled:cursor-not-allowed whitespace-nowrap">
                                Apply Range
                            </button>
                        </div>

                        <div className="flex gap-2 shrink-0 border-r border-white/[0.07] pr-4 mr-2">
                            <button onClick={reset} className="h-9 w-9 flex items-center justify-center text-zinc-500 hover:text-[#f0ede8] border border-white/[0.07] rounded-full hover:bg-white/[0.06] transition-colors" title="Start Fresh">
                                <X size={14} />
                            </button>
                            <button onClick={selectAll} className="h-9 px-4 text-xs font-semibold tracking-wide text-zinc-400 hover:text-[#f0ede8] border border-white/[0.07] rounded-full hover:bg-white/[0.06] transition-colors">All</button>
                            <button onClick={clearAll} className="h-9 px-4 text-xs font-semibold tracking-wide text-zinc-400 hover:text-red-400 border border-white/[0.07] rounded-full hover:bg-white/[0.06] transition-colors">None</button>
                        </div>
                    </div>

                    {/* Pages Grid */}
                    <div className="bg-[#1c1c1c] border border-white/[0.05] rounded-[2rem] p-6">
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-5 border-b border-white/[0.05] pb-4">
                            <div>
                                <h3 className="text-sm font-bold text-[#f0ede8]">
                                    Select Pages <span className="text-[#f0ede8]">({selectedCount} / {pageCount} selected)</span>
                                </h3>
                                ...
                                <button
                                    ...
                                    className={`group relative h-36 rounded-2xl flex flex-col items-center p-2 transition-all duration-200 border-2 ${
                                        p.selected
                                            ...
                                            : "border-white/[0.07] bg-[#1c1c1c] hover:border-white/[0.12] opacity-50 hover:opacity-70"
                                    }`}
                                >
                                    {/* Selection indicator (Top Left) */}
                                    <div className={`absolute top-2 left-2 w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all z-10 ${p.selected ? "bg-white border-white text-black" : "bg-white/[0.06] border-zinc-700 text-transparent"}`}>
                                    ...
                                    {p.selected && (
                                        <div 
                                            className="absolute top-2 right-2 w-5 h-5 bg-[#141414]/60 backdrop-blur-md text-[#f0ede8] rounded-md flex items-center justify-center hover:bg-red-500 transition-colors z-10 border border-white/10"
                                    ...
                                    <div className="flex-grow w-full relative overflow-hidden bg-white/5 rounded border border-white/5 mt-1">
                                        ...
                                        <div className="absolute top-1 right-1 bg-[#141414]/60 px-1 py-0.5 rounded text-[7px] font-black text-white/70 backdrop-blur-sm">
                                            {p.index + 1}
                                        </div>
                                    </div>
                    ...
                    {!outputUrl ? (
                        <button
                            ...
                            className={`w-full h-14 font-bold tracking-wide text-sm rounded-full flex items-center justify-center gap-3 transition-all ${selectedCount === 0 ? "bg-[#1c1c1c] text-zinc-500 border border-white/[0.07] cursor-not-allowed" : "bg-[#f0ede8] text-[#141414] hover:bg-white shadow-lg shadow-white/20"}`}
                        >
                            {isExporting
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full">
                            <button
                                onClick={downloadPdf}
                                className="h-12 px-6 bg-[#f0ede8] text-[#141414] font-bold tracking-wide text-xs sm:text-sm rounded-full flex items-center justify-center gap-2 transition-all active:scale-[0.98] shadow-xl shadow-white/10"
                            >
                                <Download size={18} /> 
                            ...
                            <button
                                ...
                                className="h-12 px-6 bg-[#1c1c1c] border border-white/[0.07] text-[#f0ede8] font-bold tracking-wide text-xs sm:text-sm rounded-full flex items-center justify-center gap-2 transition-all active:scale-[0.98] hover:bg-white/[0.06]"
                            >
                                <Share2 size={18} /> 
                            ...
                            <button
                                onClick={reset}
                                className="sm:col-span-2 h-12 px-6 bg-transparent border border-white/[0.07] text-zinc-400 hover:text-[#f0ede8] font-semibold tracking-wide text-xs sm:text-sm rounded-full flex items-center justify-center gap-2 hover:bg-[#1c1c1c] transition-all"
                            >
